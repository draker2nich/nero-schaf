...
import { VectorStrokeManager } from '../utils/vectorStroke';
...
function createLayer(type, name, options = {}) {
  ...
  const vectorStrokes = type === LAYER_TYPES.DRAWING ? new VectorStrokeManager() : null;
...
function serializeLayer(layer) {
  ...
  if (layer.vectorStrokes) {
    serialized.vectorData = layer.vectorStrokes.serialize();
  }
  return serialized;
}
...
function deserializeLayer(serialized) {
  return new Promise((resolve) => {
    ...
    let vectorStrokes = null;
    if (serialized.vectorData) {
      vectorStrokes = VectorStrokeManager.deserialize(serialized.vectorData);
    } else if (serialized.type === LAYER_TYPES.DRAWING) {
      vectorStrokes = new VectorStrokeManager();
    }
    
    const img = new Image();
    img.onload = () => { 
      ctx.drawImage(img, 0, 0, CANVAS_SIZE, CANVAS_SIZE); 
      resolve({ ...serialized, canvas, ctx, vectorStrokes, dataUrl: undefined }); 
    };
    img.onerror = () => resolve({ ...serialized, canvas, ctx, vectorStrokes, dataUrl: undefined });
...
export function useLayers(onCanvasUpdate) {
  ...
  const saveToHistory = useCallback(async () => {
    ...
    const serializedLayers = await Promise.all(currentLayers.map(serializeLayer));
  ...
  const restoreFromHistory = useCallback(async (index) => {
    ...
    const restoredLayers = await Promise.all(snapshot.layers.map(deserializeLayer));
  ...
  const clearActiveLayer = useCallback(() => {
    ...
    if (layer.vectorStrokes) layer.vectorStrokes.clear();
    setLayers(prev => [...prev]);
    saveToHistory();
    onCanvasUpdate?.(true);
